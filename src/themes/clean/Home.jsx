              ...
              {packages.filter(p => p.enabled).slice(0, 3).map((pkg, i) => {
                ...
                <div key={pkg.id} className={`rounded-xl p-6 flex flex-col border transition-all ${
                  ...
                  <div className="mt-auto pt-6">
                    <span className="text-3xl font-bold text-gray-900">{symbol}{Number(pkg.price).toFixed(2)}</span>
                    {pkg.original_price > pkg.price && (
                      <span className="text-sm text-gray-500 line-through ml-2">{symbol}{Number(pkg.original_price).toFixed(2)}</span>
                    )}
